...
class Swap {
    ...
    companion object {
        ...
        fun faceSwapAll(
            bmp1: Bitmap, // input bitmap containing first face
            bmp2: Bitmap,
            landmarksForFaces1: ArrayList<ArrayList<PointF>>, // Array of ArrayLists of PointF landmark points for each face
            ...
            if (openCvInitialized) {
                ...
                val img1 = Mat() // Mat object is opencv's "Basic Image Container"
                val img2 = Mat() // These are empty for now

                Utils.bitmapToMat(bmp1, img1) // convert bitmap to Mat object
                Utils.bitmapToMat(bmp2, img2) // Mat object take the dimensions and content of the bitmap

                Imgproc.cvtColor(img1, img1, Imgproc.COLOR_BGRA2BGR) // converts Blue Red Green Alpha to BGR
                Imgproc.cvtColor(img2, img2, Imgproc.COLOR_BGRA2BGR) // presumable because the bitmap did not contain "Alpha"

                // Perform the swap on the first set of detected landmarks (use swapSingle)
                val swappedImg1 = swapSingle(img1, img2, landmarksForFaces1[0], landmarksForFaces2[0]) // swapping the faces

                // get matrix with swap
                val bitMapResult = Bitmap.createBitmap(bmp2) // Override the bitmap 2 with the result

                // Convert the processed Mat back to Bitmap and return it
                Utils.matToBitmap(swappedImg1, bitMapResult)
                return bitMapResult
            }
            return bmp2
        }
